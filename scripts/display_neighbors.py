...
for key in left_neighbors.keys():
    ...
    if right_neighbors != None:
        overlap = len(set(left_neighbors[key].keys()) & set(right_neighbors[key].keys()))
        total = len(left_neighbors[key])
        
        print(key, overlap/total)
        sorted_words = sorted([ (sim,val) for val, sim in left_neighbors[key].items() ], reverse=True)
        top_100 = " ".join([ val for sim, val in sorted_words ])
        print(top_100)
        sorted_words = sorted([ (sim,val) for val, sim in right_neighbors[key].items() ], reverse=True)
        top_100 = " ".join([ val for sim, val in sorted_words ])
        print(top_100)
